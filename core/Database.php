...
class Database {
    ...
    public static function getInstance($dbFile = null) {
        if (self::$instance === null) { self::$instance = new self($dbFile);}
        return self::$instance;
    }
    
    private function initTables() {
        // 用户表
        $this->db->exec("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT,username TEXT UNIQUE NOT NULL,password TEXT NOT NULL,is_default_password INTEGER DEFAULT 0,created_at DATETIME DEFAULT CURRENT_TIMESTAMP)");
        // 兼容已有表，增加 is_default_password 字段
        $cols = $this->db->query("PRAGMA table_info(users)")->fetchAll();
        $hasDefaultPasswordFlag = false;
        foreach ($cols as $col) {
            if (($col['name'] ?? '') === 'is_default_password') { $hasDefaultPasswordFlag = true;break;}
        }
        if (!$hasDefaultPasswordFlag) { $this->db->exec("ALTER TABLE users ADD COLUMN is_default_password INTEGER DEFAULT 0");}
        // 服务器表
        $this->db->exec("CREATE TABLE IF NOT EXISTS servers (id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,host TEXT NOT NULL,port INTEGER DEFAULT 22,username TEXT NOT NULL,key_path TEXT,key_content TEXT,password TEXT,created_at DATETIME DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)");
        ...
        if (!$hasPassword) { $this->db->exec("ALTER TABLE servers ADD COLUMN password TEXT");}
        // 项目表
        $this->db->exec("CREATE TABLE IF NOT EXISTS projects (id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,repo_url TEXT NOT NULL,branch TEXT DEFAULT 'master',deploy_path TEXT NOT NULL,server_id INTEGER NOT NULL,git_username TEXT,git_password TEXT,pre_deploy_script TEXT,post_deploy_script TEXT,webhook_enabled INTEGER DEFAULT 0,webhook_token TEXT,created_at DATETIME DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY (server_id) REFERENCES servers(id))");
        ...
        if (!$hasGitUsername) { $this->db->exec("ALTER TABLE projects ADD COLUMN git_username TEXT");}
        ...
        $stmt->execute(['admin']);
        if ($stmt->fetchColumn() == 0) { $password = password_hash('admin', PASSWORD_DEFAULT); $stmt = $this->db->prepare("INSERT INTO users (username, password, is_default_password) VALUES (?, ?, 1)"); $stmt->execute(['admin', $password]); } else { $user = $this->fetchOne("SELECT id, password, is_default_password FROM users WHERE username = ?", ['admin']); if ($user && ($user['is_default_password'] ?? 0) == 0) { if (password_verify('admin', $user['password'])) { $this->update('users', ['is_default_password' => 1], 'id = ?', [$user['id']]); } } }
    }
    ...
    private function validateTableName($table) {
        if (!in_array($table, self::$allowedTables, true)) { throw new InvalidArgumentException("无效的表名: {$table}"); }
        return $table;
    }
    ...
    private function validateFieldNames($table, $fields) {
        if (!isset(self::$allowedFields[$table])) { throw new InvalidArgumentException("表 {$table} 没有定义字段白名单"); }
        $allowed = self::$allowedFields[$table];
        foreach ($fields as $field) {
            if (!in_array($field, $allowed, true)) { throw new InvalidArgumentException("表 {$table} 中不允许使用字段: {$field}"); }
        }
        return true;
    }
    ...
    private function validateWhereClause($table, $where) {
        if (empty($where) || !is_string($where)) { throw new InvalidArgumentException("WHERE子句不能为空且必须是字符串"); }
        $where = trim($where);
        // 禁止SQL注释符号（防止注释注入攻击）
        ...
        // 禁止引号、分号等特殊字符
        if (preg_match('/[\'";`]/', $where)) { throw new InvalidArgumentException("WHERE子句不能包含引号或分号"); }
        // 严格验证格式：只允许 "字段名 = ?" 或 "字段名 = :参数名"
        if (!preg_match('/^([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([?:][a-zA-Z0-9_]*)$/', $where, $matches)) { throw new InvalidArgumentException("WHERE子句格式无效，只允许: 字段名 = ? 或 字段名 = :参数名"); }
        // 验证字段名是否在白名单中
        $fieldName = $matches[1];
        if (!isset(self::$allowedFields[$table]) || !in_array($fieldName, self::$allowedFields[$table], true)) { throw new InvalidArgumentException("WHERE子句中的字段名不在白名单中: {$fieldName}"); }
        return $where;
    }
    ...
    public function insert($table, $data) {
        ...
        // 加密敏感字段
        if ($table === 'servers') { $encryptFields = ['password', 'key_content']; if (class_exists('SecureStorage')) { $data = SecureStorage::encryptFields($data, $encryptFields); } } elseif ($table === 'projects') { $encryptFields = ['git_password']; if (class_exists('SecureStorage')) { $data = SecureStorage::encryptFields($data, $encryptFields); } }
        
        // 使用白名单字段名构建SQL（防止注入）
        $validFields = [];
        $validData = [];
        foreach ($fields as $field) {
            if (in_array($field, self::$allowedFields[$table], true)) { $validFields[] = $field; $validData[$field] = $data[$field]; }
        }
        if (empty($validFields)) { throw new InvalidArgumentException("没有有效的字段可以插入"); }
    ...
    public function update($table, $data, $where, $whereParams = []) {
        ...
        $validData = [];
        foreach ($fields as $field) {
            if (in_array($field, self::$allowedFields[$table], true)) { $setParts[] = "{$field} = :{$field}"; $validData[$field] = $data[$field]; }
        }
        if (empty($setParts)) { throw new InvalidArgumentException("没有有效的字段可以更新"); }
        ...
        if (strpos($where, '?') !== false) { $wherePlaceholder = preg_replace_callback('/\?/', function() use (&$paramIndex, &$whereParamsNamed, $whereParams) { $paramName = ':where_param_' . $paramIndex++; if (isset($whereParams[$paramIndex - 1])) { $whereParamsNamed[$paramName] = $whereParams[$paramIndex - 1]; } return $paramName; }, $where); } else { $whereParamsNamed = $whereParams; }
